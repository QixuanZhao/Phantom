...
void main() {
    ...
    vec4 lightSpacePosition = lightPV * vec4(fragmentPosition, 1.0);

    vec3 projCoord = lightSpacePosition.xyz / lightSpacePosition.w;
    projCoord = 0.5 * projCoord + 0.5;

    if (!samplingEnabled) {
        red = texture(depthMap, projCoord);
        return;
    }

    switch (samplingMethod) {
    case 0: red = pcf(projCoord); break;
    case 1: red = pds(projCoord); break;
    }
}
...
float pcf (vec3 projCoord) {
    ...
    float spread = sqrt(samplingSpread) / (filterSize * sqrt(tSize.x * tSize.y));

    if (filterSize % 2 == 0) { // even
        int halfSize = filterSize / 2;
        for (int i = -halfSize; i < halfSize; i++) {
            for (int j = -halfSize; j < halfSize; j++) {
                shadow += texture(depthMap, projCoord + vec3((vec2(i, j) + 0.5) * spread, 0.0));
            }
        }
    } else { // odd
        ...
        for (int i = -projection; i <= projection; i++) {
            for (int j = -projection; j <= projection; j++) {
                shadow += texture(depthMap, projCoord + vec3(vec2(i, j) * spread, 0.0));
            }
        }
    }
...
float pds (vec3 projCoord) {
    float shadow = 0.0;
    ivec2 tSize = textureSize(depthMap, 0);
    float spread = sqrt(samplingSpread) / length(tSize);
    ...
    for (int i = 0; i < poissonDisk.length(); i++) {
        ...
        shadow += texture(depthMap, projCoord + vec3(offset * spread, 0.0));
